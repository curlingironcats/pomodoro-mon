...
time_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "SETTINGS\\timetracker.txt")
#window settings file path
window_settings_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "SETTINGS\\windowsettings.txt")
...
# read the X and Y position of the window from when last closed
try:
...
music_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "SETTINGS\\musicplayer.txt")

with open(music_file_path, "r") as music_file:
    if os.path.getsize(music_file_path) > 0:
        stored_music = music_file.read()
    else:
